...
test("buildCustomerProfile should summarize customer spend and order count", () => {
  ...
  const profile = hooks.buildCustomerProfile({
    ...
    orders: [{ id: "ord-1", userId: "user-1", customerName: "أحمد", total: 20, status: "processing", createdAt: "2026-04-01T10:00:00Z" }],
    serviceOrders: [{ id: "srv-1", userId: "user-1", serviceName: "صيانة", total: 15, status: "completed", createdAt: "2026-04-02T10:00:00Z" }]
  });
...
test("collectOrderActivity should merge physical and digital activity in reverse chronological order", () => {
  const hooks = loadHooks();
  const activity = hooks.collectOrderActivity(
    ...
    [{ id: "srv-1", userId: "user-1", serviceName: "اشتراك", total: 15, createdAt: "2026-04-03T10:00:00Z" }]
  );
