    ...
    <div className="surface-panel section-shell">
      <div className="section-shell-head">
        <div className="section-shell-copy">
          <h2>{hasSubCategories ? 'منتجات مباشرة داخل هذه الفئة' : 'منتجات هذه الفئة'}</h2>
          <p>
            {hasSubCategories
              ? 'هذه المنتجات مرتبطة بهذه الفئة مباشرة وتظهر هنا بجانب الأقسام الفرعية عند توفرها.'
      ...
      <div className="balanced-card-grid">
        {products.map((product) => (
          <ProductCard key={product.id} product={mapCategoryProductCard(product, categoryName)} />
        ))}
      </div>
    </div>
  );
