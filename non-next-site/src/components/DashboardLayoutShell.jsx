...
/**
 * Loads the current wallet snapshot for the authenticated user.
 *
 * @param {string} userId
...
/**
 * Renders the customer dashboard layout for the non-Next copy.
 *
...
export default function DashboardLayoutShell({ children }) {
  ...
  useEffect(() => {
    ...
    /**
     * Refreshes dashboard counters and wallet balance.
     *
     * @param {string} userId
    ...
    async function initialize() {
      ...
      const notificationsChannel = supabase
        ...
        .on(
          ...
          () => {
            void refreshLiveStats(currentUser.id);
          }
        )
        .subscribe();
    ...
    /**
     * Refreshes notification counters after local mutations.
     *
     * @returns {Promise<void>}
     */
  ...
  const navItems = NAV_ITEMS.map((item) => {
    if (item.href === "/dashboard/notifications") {
      return { ...item, badge: unreadNotifications > 99 ? "99+" : unreadNotifications || "" };
    }

    if (item.href === "/dashboard/favorites") {
      return { ...item, badge: favoriteCount > 99 ? "99+" : favoriteCount || "" };
    }
      ...
      <div className="container">
        <div className="dashboard-hero">
          ...
          <div className="dashboard-hero-actions">
            ...
            <button onClick={() => void handleLogout()} className="btn btn-danger">
              🚪 خروج
            </button>
          </div>
        </div>
