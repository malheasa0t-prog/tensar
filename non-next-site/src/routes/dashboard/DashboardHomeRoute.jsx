...
async function fetchDashboardHomeSnapshot() {
  ...
  const activeSyncOrders = orders.filter(
    (order) => order.external_order_id && ["pending", "processing", "in_progress", "partial"].includes(order.status)
  );
...
/**
 * Renders the dashboard overview route in the non-Next copy.
 *
 * @returns {JSX.Element}
 */
export default function DashboardHomeRoute() {
  ...
  useEffect(() => {
    ...
    /**
     * Loads the initial dashboard snapshot and binds realtime updates.
     *
    ...
    async function initialize() {
      ...
      const ordersChannel = supabase
        ...
        .on(
          "postgres_changes",
          { event: "*", schema: "public", table: "service_orders", filter: `user_id=eq.${snapshot.userId}` },
          () => {
      ...
      const notificationsChannel = supabase
        ...
        .on(
          "postgres_changes",
          { event: "*", schema: "public", table: "notifications", filter: `user_id=eq.${snapshot.userId}` },
          () => {
            void refreshSnapshot();
          }
        )
  ...
  return (
    <div className="dash-home">
      <div className="dash-home-top">
        <div className="dash-sync-panel">
          ...
          <div className="dash-sync-metrics">
            ...
            <div>
              <span>آخر تحديث</span>
              <strong>{syncMeta.lastUpdate ? new Date(syncMeta.lastUpdate).toLocaleString("ar-JO") : "لا يوجد"}</strong>
            </div>
          </div>
        </div>
      ...
      <div className="dash-orders-card">
        ...
        {loading ? (
          <div className="dash-loading">جارٍ التحميل...</div>
