...
test("optimizeImageSrc should proxy approved provider images through the storefront transformer", () => {
  const optimized = new URL(
    optimizeImageSrc({
      quality: 72,
      src: "https://serva-s.com/images/sample.jpg",
      width: 960,
    }),
    "https://tensr.systems"
  );

  assert.equal(optimized.pathname, "/api/img");
  assert.equal(optimized.searchParams.get("url"), "https://serva-s.com/images/sample.jpg");
  assert.equal(optimized.searchParams.get("w"), "960");
  assert.equal(optimized.searchParams.get("q"), "72");
  assert.equal(optimized.searchParams.get("format"), "webp");
});
