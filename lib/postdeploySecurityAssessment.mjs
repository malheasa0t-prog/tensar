...
function evaluateHeaderProbe(input) {
  ...
  if (
    ...
    && missingHeaders[0] === "strict-transport-security"
    && input.responseUrl.startsWith("https://")
  ) {
    return {
      details: "Strict-Transport-Security was not observed by the probe. Verify edge/domain HSTS manually.",
      status: STATUS_WARN,
    };
  }
