...
export const INVALID_DEPOSIT_AMOUNT_MESSAGE = "[DPG-101] مبلغ الشحن غير صالح.";
export const MIN_DEPOSIT_AMOUNT_MESSAGE = `[DPG-101] الحد الأدنى للشحن ${MIN_DEPOSIT_AMOUNT} د.أ`;
export const MAX_DEPOSIT_AMOUNT_MESSAGE = `[DPG-101] الحد الأقصى للشحن ${MAX_DEPOSIT_AMOUNT} د.أ`;
...
export function validateDepositAmount(amount) {
  ...
  // Reject sub-cent precision (e.g. 15.005). The column is numeric(12,2), so a
  // 3rd decimal is silently rounded on insert and then never equals the float
  // the Orange Money SMS matcher compares against — leaving it unmatchable.
...
export function buildDepositProofObjectPath({ userId, fileName = "", now = Date.now() }) {
  ...
  if (!normalizedUserId) {
    throw new Error("[DPG-102] معرّف المستخدم مطلوب لرفع إثبات الإيداع.");
  }
