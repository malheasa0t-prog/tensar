...
test("getHeroVisualItems should prefer featured categories and fill remaining stage slots", () => {
...
test("getHeroVisualItems should fall back to default cards when no categories are available", () => {
  const result = getHeroVisualItems(null);

  assert.equal(result.primaryItem.id, "hero-builds");
  assert.equal(result.floatingItems.length, 1);
  assert.equal(result.floatingItems[0].id, "hero-repairs");
});
