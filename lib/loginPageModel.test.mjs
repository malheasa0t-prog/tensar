...
test("validateRecoveryEmail and validatePasswordResetForm should cover missing and invalid values", () => {
  ...
  assert.equal(
    validatePasswordResetForm({ password: "1234567890", confirmPassword: "0987654321" }),
    "كلمتا المرور غير متطابقتين."
  );
  assert.equal(validatePasswordResetForm({ password: "Strong1Pwd!", confirmPassword: "Strong1Pwd!" }), "");
});
...
test("buildOAuthSignInOptions should request account selection for Google only", () => {
  assert.deepEqual(
    buildOAuthSignInOptions({
      provider: "google",
      redirectTo: "https://tensr.systems/auth/callback",
    }),
    {
      redirectTo: "https://tensr.systems/auth/callback",
