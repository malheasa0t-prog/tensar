...
const DEFAULT_SITE_DESCRIPTION =
  "تسوق الخدمات الرقمية واحجز خدمات الصيانة من TechZone بتجربة عربية سريعة وواضحة.";
...
function buildUtilityMetadata(pathname) {
  const routeMap = {
    ...
    "/deposit": {
      title: "إيداع الرصيد",
      description: "أنشئ طلب إيداع للمحفظة عبر Orange Money باستخدام المبلغ ورقم الهاتف المحول منه.",
      robots: DEFAULT_PRIVATE_ROBOTS,
      breadcrumbItems: [
        { href: "/", label: "الرئيسية" },
        { label: "إيداع الرصيد" },
...
export function getRouteSeoDefaults({ pathname = "/", siteSettings = {} } = {}) {
  ...
  if (normalizedPath in { "/services": 1, "/contact": 1 }) {
