...
export function buildDocumentTitle({ brandName = "TechZone", pageTitle = "" } = {}) {
  ...
  return resolvedPageTitle
    ? `${resolvedBrandName} | ${resolvedPageTitle}`
    : resolvedBrandName;
}
...
function buildCatalogMetadata(pathname) {
  const routeMap = {
    ...
    "/accessories": {
      title: "الإكسسوارات",
      description: "اكتشف إكسسوارات الكمبيوتر وملحقات الأجهزة بعرض أوضح وتصفح أسرع.",
      breadcrumbItems: [
        { href: "/", label: "الرئيسية" },
        { label: "الإكسسوارات" },
      ],
    ...
    "/subscriptions": {
      title: "شحن واشتراكات",
      description: "بطاقات الشحن والاشتراكات الرقمية داخل كتالوج منظم وأسرع في الوصول.",
    },
    "/contact": {
      title: "اتصل بنا",
...
export function getRouteSeoDefaults({ pathname = "/", siteSettings = {} } = {}) {
  ...
  if (
    normalizedPath in
    { "/products": 1, "/services": 1, "/accessories": 1, "/subscriptions": 1, "/contact": 1 }
  ) {
...
export function mergeSeoMetadata({ routeMetadata = {}, pageMetadata = {} } = {}) {
  ...
  const structuredData = Array.isArray(pageData.structuredData)
    ? pageData.structuredData.filter(Boolean)
    : [];
  ...
  return {
    ...
    canonicalPath:
      cleanText(pageData.canonicalPath) ||
      cleanText(routeMetadata.canonicalPath) ||
      "/",
  };
}
