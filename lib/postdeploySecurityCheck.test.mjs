...
test("evaluateHeaderProbe should warn when only HSTS is missing over HTTPS", () => {
  const result = evaluateHeaderProbe({
    headers: {
      "content-security-policy": "default-src 'self'",
      "x-content-type-options": "nosniff",
      "x-frame-options": "DENY",
    },
    responseUrl: "https://tensr.systems/",
  });

  assert.equal(result.status, "warn");
  assert.match(result.details, /Strict-Transport-Security was not observed/i);
});
