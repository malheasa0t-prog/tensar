...
const OAUTH_QUERY_PARAMS_BY_PROVIDER = Object.freeze({
  google: Object.freeze({ prompt: "select_account" }),
});
...
/**
 * Builds the OAuth options for a social sign-in attempt.
 *
 * @param {{ provider?: unknown, redirectTo?: unknown }} input
 * @returns {{ redirectTo?: string, queryParams?: Record<string, string> }}
 */
export function buildOAuthSignInOptions(input) {
  const provider = String(input?.provider || "").trim().toLowerCase();
  const redirectTo = String(input?.redirectTo || "").trim();
  const queryParams = OAUTH_QUERY_PARAMS_BY_PROVIDER[provider];
  const options = {};
