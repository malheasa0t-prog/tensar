-- Migration: 2026-05-30-03 - unified human-friendly order numbers
--
-- Purpose:
--   Give every product order, service order, and repair booking a single
--   human-friendly display number sequence that starts at #2000.
--
-- Notes:
--   - Existing rows are backfilled in chronological order.
--   - New rows read from the shared public.order_display_number_seq sequence.
...
begin
  select greatest(
    ...
    coalesce((select max(display_number) from public.service_orders), 0),
    coalesce((select max(display_number) from public.repair_bookings), 0)
  ) into v_max;

  if v_max >= 2000 then
    perform setval('public.order_display_number_seq', v_max, true);
  end if;
  -- If v_max < 2000, sequence stays at its default start (2000)
end;
$$;
