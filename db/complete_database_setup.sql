...
-- ============================================================
-- Current application compatibility additions
-- Generated from code inspection on 2026-05-24.
-- ============================================================

-- Keep role checks aligned with the admin shell roles.
alter table public.app_users
  drop constraint if exists app_users_role_check;

alter table public.app_users
  add constraint app_users_role_check
  check (role in ('customer', 'user', 'seller', 'admin', 'super_admin', 'technician', 'employee'));
...
-- Keep wallet transaction metadata aligned with newer wallet rollback flows.
...
-- The current repair booking form supports remote maintenance.
alter table public.repair_bookings
  drop constraint if exists repair_bookings_mode_check;

alter table public.repair_bookings
  add constraint repair_bookings_mode_check
  check (mode in ('delivery', 'pickup', 'onsite', 'remote'));

-- Backfill profile emails for admin/customer loading.
...
-- ============================================================
-- Additional admin tables used by the current admin shell
-- ============================================================
...
create table if not exists public.refund_requests (
  ...
  order_type text not null default 'product'
    check (order_type in ('product', 'digital', 'service', 'repair')),
...
create index if not exists idx_refund_requests_user_id on public.refund_requests(user_id, created_at desc);
...
create table if not exists public.platform_updates (
  id uuid primary key default gen_random_uuid(),
  title text not null,
  badge text not null default 'جديد',
  summary text,
  content text,
  image text,
  cta_label text,
...
create index if not exists idx_platform_updates_published
  on public.platform_updates(is_published, is_pinned desc, created_at desc);

create table if not exists public.seller_category_discounts (
  id uuid primary key default gen_random_uuid(),
  ...
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now(),
  constraint seller_category_discounts_unique unique (user_id, category_id)
);

create index if not exists idx_seller_category_discounts_user_id
  on public.seller_category_discounts(user_id);
create index if not exists idx_seller_category_discounts_category_id
  on public.seller_category_discounts(category_id);

-- updated_at triggers for current admin tables.
...
drop trigger if exists trg_seller_category_discounts_updated_at on public.seller_category_discounts;
create trigger trg_seller_category_discounts_updated_at
before update on public.seller_category_discounts
for each row execute function public.set_updated_at_now();

alter table public.refund_requests enable row level security;
alter table public.platform_updates enable row level security;
alter table public.seller_category_discounts enable row level security;
...
drop policy if exists refund_requests_delete_admin on public.refund_requests;
create policy refund_requests_delete_admin
on public.refund_requests for delete
using (public.is_current_admin());

-- platform_updates policies
drop policy if exists platform_updates_public_read_published on public.platform_updates;
create policy platform_updates_public_read_published
on public.platform_updates for select
using (is_published = true or public.is_current_admin());

drop policy if exists platform_updates_admin_manage on public.platform_updates;
create policy platform_updates_admin_manage
on public.platform_updates for all
using (public.is_current_admin())
with check (public.is_current_admin());

-- seller_category_discounts policies
drop policy if exists seller_category_discounts_admin_read on public.seller_category_discounts;
create policy seller_category_discounts_admin_read
on public.seller_category_discounts for select
using (public.is_current_admin());

drop policy if exists seller_category_discounts_admin_manage on public.seller_category_discounts;
create policy seller_category_discounts_admin_manage
on public.seller_category_discounts for all
using (public.is_current_admin())
with check (public.is_current_admin());

-- ============================================================
-- Auth/profile sync functions with user_profiles.email support
-- ============================================================
...
begin
  ...
  v_email := coalesce(nullif(lower(trim(new.email)), ''), nullif(v_email, ''));
  v_name := coalesce(nullif(trim(new.full_name), ''), 'User-' || right(replace(new.user_id::text, '-', ''), 6));
...
-- ============================================================
-- Inventory RPCs used by checkout rollback flow
-- ============================================================
...
begin
  if nullif(trim(coalesce(p_product_id, '')), '') is null or p_quantity is null or p_quantity <= 0 then
    return false;
  end if;
  ...
  if coalesce(v_product.status, 'active') <> 'active' or coalesce(v_product.quantity, 0) < p_quantity then
    return false;
  end if;

  v_next_quantity := v_product.quantity - p_quantity;

  update public.products
  set
    quantity = v_next_quantity,
    sold = coalesce(sold, 0) + p_quantity,
    status = case when v_next_quantity = 0 then 'out_of_stock' else status end,
    updated_at = now()
  where id = p_product_id;
...
begin
  ...
  v_next_quantity := coalesce(v_product.quantity, 0) + p_quantity;

  update public.products
  set
    quantity = v_next_quantity,
    sold = greatest(0, coalesce(sold, 0) - p_quantity),
    status = case when coalesce(status, '') = 'out_of_stock' and v_next_quantity > 0 then 'active' else status end,
    updated_at = now()
  where id = p_product_id;
...
revoke all on function public.deduct_inventory(text, integer) from public, anon, authenticated;
revoke all on function public.restore_inventory(text, integer) from public, anon, authenticated;
grant execute on function public.deduct_inventory(text, integer) to service_role;
grant execute on function public.restore_inventory(text, integer) to service_role;

-- ============================================================
-- Admin and provider RPCs used by Cloudflare Functions/admin proxy
-- ============================================================
...
begin
  ...
  set
    status = 'approved',
    reviewed_by = p_admin_user_id::text,
    reviewed_at = now(),
    updated_at = now()
  ...
  values (
    ...
    'تمت إضافة ' || v_deposit.amount::text || ' د.أ إلى محفظتك.',
    'success',
    'wallet_transaction',
    v_tx_id::text
  );
...
begin
  ...
  from public.user_profiles
  where user_id::text = v_target or id::text = v_target or lower(coalesce(email, '')) = lower(v_target)
  limit 1
  for update;
  ...
  from public.app_users
  where id = v_target or auth_user_id::text = v_target or lower(coalesce(email, '')) = lower(v_target)
  limit 1
  for update;
...
revoke all on function public.admin_approve_deposit(uuid, uuid) from public, anon, authenticated;
revoke all on function public.admin_toggle_customer_status(text) from public, anon, authenticated;
revoke all on function public.sync_service_order_status_tx(text, text, text, integer, integer) from public, anon, authenticated;
revoke all on function public.release_service_order_wallet(text, text) from public, anon, authenticated;
revoke all on function public.merge_service_order_metadata(text, jsonb) from public, anon, authenticated;
revoke all on function public.admin_set_order_status(text, text, text, uuid, text) from public, anon, authenticated;

grant execute on function public.admin_approve_deposit(uuid, uuid) to service_role;
grant execute on function public.admin_toggle_customer_status(text) to service_role;
grant execute on function public.sync_service_order_status_tx(text, text, text, integer, integer) to service_role;
grant execute on function public.release_service_order_wallet(text, text) to service_role;
grant execute on function public.merge_service_order_metadata(text, jsonb) to service_role;
grant execute on function public.admin_set_order_status(text, text, text, uuid, text) to service_role;

-- ============================================================
-- Storage buckets used by admin uploads and deposit proof uploads
-- ============================================================

insert into storage.buckets (id, name, public, file_size_limit, allowed_mime_types)
values
  ('product-images', 'product-images', true, 4194304, array['image/jpeg', 'image/jpg', 'image/png', 'image/webp']),
  ('category-images', 'category-images', true, 3145728, array['image/jpeg', 'image/jpg', 'image/png', 'image/webp']),
  ('repair-service-images', 'repair-service-images', true, 3145728, array['image/jpeg', 'image/jpg', 'image/png', 'image/webp']),
  ('deposits', 'deposits', false, 5242880, array['image/jpeg', 'image/jpg', 'image/png', 'image/webp'])
on conflict (id) do update
set
  public = excluded.public,
  file_size_limit = excluded.file_size_limit,
...
on storage.objects for select
to public
using (bucket_id in ('product-images', 'category-images', 'repair-service-images'));
...
to authenticated
with check (
  bucket_id in ('product-images', 'category-images', 'repair-service-images')
...
-- Table grants for objects created after the original schema-wide grants.
grant select on public.platform_updates to anon, authenticated;
grant all on public.refund_requests to authenticated;
grant all on public.platform_updates to authenticated;
grant all on public.seller_category_discounts to authenticated;

alter table public.refund_requests replica identity full;
alter table public.platform_updates replica identity full;
alter table public.seller_category_discounts replica identity full;
...
select public.enable_realtime_for_table('public.refund_requests'::regclass);
select public.enable_realtime_for_table('public.platform_updates'::regclass);
select public.enable_realtime_for_table('public.seller_category_discounts'::regclass);
