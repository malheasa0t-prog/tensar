...
const STOCK_ALERT_SAVE_ERROR_MESSAGE = "تعذر تفعيل تنبيه التوفر حالياً.";
...
async function authenticateStockAlertRequest(request, env) {
  ...
  if (!token) {
    return { error: "سجل الدخول أولاً لتفعيل تنبيه التوفر.", user: null };
  }
  ...
  if (error || !user) {
    return { error: "جلسة تسجيل الدخول غير صالحة.", user: null };
  }
...
async function handlePost(request, env) {
  ...
  if (productResponse.error || !productResponse.data) {
    return errorResponse("المنتج غير موجود أو لم يعد متاحاً.", 404);
  }

  if (!isStockAlertEligibleProduct(productResponse.data)) {
    return errorResponse("المنتج متوفر الآن بالفعل.", 409);
  }
...
export async function onRequest(context) {
  ...
  const response = method === "POST" ? await handlePost(context.request, context.env) : errorResponse("Method not allowed", 405);
